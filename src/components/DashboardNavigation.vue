      ...
      <div class="h-full flex flex-col p-6 lg:p-6">
        ...
        <div class="mb-8">
          <div class="flex items-center gap-2 mb-2">
            ...
            />
            <h1 class="text-xl font-bold text-foreground whitespace-nowrap leading-tight -mt-1">HOMELAB</h1>
          </div>
        ...
        <div class="flex-1 space-y-2">
          ...
          >
            <component :is="tab.icon" :class="['w-5 h-5', activeTab === tab.id ? 'text-primary' : '']" />
            <div class="flex-1 min-w-0">
              <div class="font-medium truncate">{{ tab.label }}</div>
              <div class="text-xs opacity-75 truncate">{{ tab.description }}</div>
            </div>
            <Badge v-if="activeTab === tab.id" variant="secondary" class="text-xs">
              Active
            </Badge>
          </button>
        </div>
