<template>
  <div class="space-y-6">
    ...
    <div v-else class="space-y-6">
      <!-- System Overview -->
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card class="p-4 bg-gradient-to-br from-card/80 to-card/60 border-border/50 backdrop-blur-sm">
          <div class="flex items-center justify-between">
            <div>
        ...
        <Card class="p-4 bg-gradient-to-br from-card/80 to-card/60 border-border/50 backdrop-blur-sm">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-lg font-semibold text-foreground">{{ stats.loadAverage.join(', ') }}</p>
            </div>
...
const currentCpu = computed(() => metrics.value.length > 0 ? metrics.value[metrics.value.length - 1].cpu : 0)
const currentMemory = computed(() => metrics.value.length > 0 ? metrics.value[metrics.value.length - 1].memory : 0)
const currentNetwork = computed(() => metrics.value.length > 0 ? metrics.value[metrics.value.length - 1].network : 0)
const currentDisk = computed(() => metrics.value.length > 0 ? metrics.value[metrics.value.length - 1].disk : 0)
const currentTemp = computed(() => metrics.value.length > 0 ? metrics.value[metrics.value.length - 1].temperature : 0)
