<template>
  <div class="space-y-6">
    <!-- Error State -->
    <div v-if="systemError" class="space-y-6">
      <div class="bg-destructive/10 border border-destructive/20 rounded-lg p-6 text-center">
        <div class="flex items-center justify-center gap-2 text-destructive mb-2">
          <AlertCircle class="w-5 h-5" />
          <span class="font-medium">Failed to load system data</span>
        </div>
        <p class="text-sm text-muted-foreground mb-4">{{ systemError }}</p>
    ...
    <!-- Loading State -->
    <div v-else-if="isLoading || systemLoading" class="space-y-6">
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-lg font-semibold text-foreground">{{ formatUptime(uptime) }}</p>
            </div>
        ...
        >
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-lg font-semibold text-foreground">{{ processes?.length || 0 }}</p>
              <p class="text-xs text-muted-foreground">{{ processes?.filter(p => p.status === 'running').length || 0 }} running</p>
            </div>
...
// Use the system monitoring composable
const {
...
const currentCpu = computed(() => cpuUsage.value)
const currentMemory = computed(() => memoryUsage.value)
const currentNetwork = computed(() =>
  metrics.value.length > 0 ? metrics.value[metrics.value.length - 1].network : 0
)
const currentDisk = computed(() => diskUsage.value)
